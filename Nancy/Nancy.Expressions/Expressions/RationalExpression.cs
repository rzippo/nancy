...
public abstract record RationalExpression : IGenericExpression<Rational>, IVisitableRational
{
    ...
    /// </remarks>
    public virtual bool Equals(RationalExpression? other)
        => other is not null;

    /// <inheritdoc />
    /// <remarks>
    /// Seeded by the concrete <see cref="Type"/>, so every expression's hash differs from the hash of the bare value it wraps, a <see cref="RationalNumberExpression"/> from its own <see cref="Rational"/> included.
    /// </remarks>
    public override int GetHashCode()
        => HashCode.Combine(274127119, GetType());

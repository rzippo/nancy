...
public class CurveToUpperNonIncreasing
{
    private readonly ITestOutputHelper _testOutputHelper;

    public CurveToUpperNonIncreasing(ITestOutputHelper testOutputHelper)
    {
    ...
    public static IEnumerable<object[]> ToUpperNonIncreasingTestCases
        => TestCases.ToXUnitTestCases();

    [Theory]
    [MemberData(nameof(ToUpperNonIncreasingTestCases))]
    public void ToUpperNonIncreasingEquivalence(Curve operand)
    {
        _testOutputHelper.WriteLine($"operand: {operand.ToCodeString()}");

        // Compute through Nancy
        var nancyResult = operand.ToUpperNonIncreasing();
        ...
        var toUpperNonIncExp = Expressions.ToUpperNonIncreasing(expressionOperand);
        var expressionResult = toUpperNonIncExp.Compute();
    ...
    [Theory]
    [MemberData(nameof(ToUpperNonIncreasingTestCases))]
    public void ToUpperNonIncreasingConcreteAndInstanceOverloadsComputeProjection(Curve operand)
    {
        var expected = operand.ToUpperNonIncreasing();
        var concreteExpression = Expressions.ToUpperNonIncreasing(operand);
        var instanceExpression = operand.ToExpression().ToUpperNonIncreasing();

        Assert.IsType<ToUpperNonIncreasingExpression>(concreteExpression);
        Assert.IsType<ToUpperNonIncreasingExpression>(instanceExpression);
